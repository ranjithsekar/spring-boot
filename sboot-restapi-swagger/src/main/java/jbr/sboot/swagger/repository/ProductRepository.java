...
public interface ProductRepository extends JpaRepository<ProductModel, Long> {
  ...
  @Query(value = "SELECT id, name, category, price FROM ProductModel WHERE price BETWEEN ?1 AND ?2")
  List<ProductModel> getProductsByPriceMinMax(String minPrice, String maxPrice);

  @Query(value = "SELECT id, name, category, price FROM ProductModel WHERE price >= ?1")
